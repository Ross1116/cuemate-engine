...
This note captures the current state of audio decode ownership after the Milestone 5 cleanup pass.

## Current state

- The repository does **not** currently track a shared-decoding refactor as an active milestone item.
- Decode-heavy paths still live close to the service or runtime that owns the corresponding inference flow.
...
- A shared decode abstraction would only be worth the complexity if one of these becomes true:
